...
class PosePipeline:
    ...
    def __init__(self, T_base_link, T_link_camera, joint_axis: str = "z"):
        ...
        if T_base_link.shape != (4, 4):
            raise ValueError(f"T_base_link 는 4x4 여야 합니다. 받은 shape={T_base_link.shape}")
        if T_link_camera.shape != (4, 4):
            raise ValueError(f"T_link_camera 는 4x4 여야 합니다. 받은 shape={T_link_camera.shape}")
        if joint_axis not in _ROT:
            raise ValueError(f"joint_axis 는 'x'|'y'|'z' 여야 합니다. 받은 값={joint_axis!r}")
    ...
    def T_base_link(self) -> np.ndarray:
        ...
        return self._T_base_link0 @ make_T(R_joint, [0.0, 0.0, 0.0])
