#!/usr/bin/env python3
"""polygon_action_server — 문제 6: DrawPolygon 액션 서버.
...
시험해보기 (삼각형·오각형·팔각형을 각각 캡처)
  ...
  실행 중 Ctrl+C 를 누르면 ros2 CLI 가 취소 요청을 보낸다 → 거북이가 "즉시" 멈춰야 한다.

왜 ReentrantCallbackGroup + MultiThreadedExecutor 인가
  execute 콜백은 다각형을 다 그릴 때까지(수 초~수십 초) 도는 "긴 콜백"이다.
  SingleThreadedExecutor 라면 그동안 다른 콜백이 전혀 실행되지 못한다. 그러면
    - /turtle1/pose 구독 콜백이 안 돌아 self._latest_pose 가 갱신되지 않고 (→ 전진 거리 판정 불가)
    - 취소 요청(cancel) 콜백도 안 돌아 goal_handle.is_cancel_requested 가 영원히 False 다.
  그래서 (1) 콜백들을 ReentrantCallbackGroup 에 넣어 "동시 실행 가능"으로 표시하고,
  (2) MultiThreadedExecutor 로 실제 스레드를 여러 개 돌린다. 둘 중 하나만 하면 효과가 없다.

거리·각도 판정을 시간이 아니라 /turtle1/pose 로 하는 이유
  "1 m/s 로 2초"는 이론값이다. 실제로는 발행 지연·turtlesim 주기(62.5Hz)·벽 충돌 때문에 어긋난다.
  pose 로 "실제 이동량"을 재면 result.total_distance 도 실측치가 되고, 벽에 막혔을 때 타임아웃으로
  abort 할 수 있다.
"""
...
class PolygonActionServer(Node):
    ...
    def __init__(self):
        ...
        # config/params.yaml (문제 9) 에서 주입되는 값들
        ...
        # execute 가 도는 동안에도 pose 가 계속 갱신되도록 같은 reentrant 그룹에 둔다.
    ...
    # ------------------------------------------------------------------ 실행
    def _stop(self):
        self._cmd_pub.publish(Twist())   # 0 속도 발행 = 즉시 정지
    ...
    def _drive_straight(self, goal_handle, length, v_max, period, timeout_factor):
        ...
        while True:
            ...
            # 끝에 가까워지면 감속해 오버슈트를 줄인다 (간단한 P 제어).
...
def main(args=None):
    ...
    # 스레드 수는 최소 2 이상이어야 execute + (pose 콜백 / cancel 콜백) 이 동시에 돈다.
